...
public:
    ...
    void AddRoundKey(Types::Blob key, int offset)
    {
    ...
    Types::Blob Data(void)
    {
        return Types::Blob(_matrix, sizeof(_matrix));
    }
};
...
Types::Blob ExpandKey(Types::Blob keyBlob, int rounds)
{
    int expansionRounds = 4 * (rounds + 1);
    Word result[expansionRounds];
    Word temp;
    ...
    while (i < Nk) {
        int base = i * 4;
        result[i] = Word::From(key[base + 0], key[base + 1], key[base + 2], key[base + 3]);
        i++;
    }
    ...
    while (i < expansionRounds) {
        temp = result[i - 1];
        if ((i % Nk) == 0)
            temp = temp.RotWord().SubWord().Xor(Word::From(Rcon[(i / Nk) - 1]));
        else if ((Nk > 6) && ((i % Nk) == 4))
            temp = temp.SubWord();
...
Types::Blob AES::Encrypt(Types::Blob data)
{
    State state(data);
...
Types::Blob AES::Decrypt(Types::Blob data)
{
    State state(data);
    state.AddRoundKey(_expandedKey, _rounds);
